...
async def message_commands_reg(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        msg_text, keyboard = await generate_profile_register(message.from_user.id, data['dict_user_data'], "cmd_register_step_1")
...
async def callback_commands_reg(call: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        msg_text, keyboard = await generate_profile_register(call.message.chat.id, data['dict_user_data'], "cmd_register_step_1")
