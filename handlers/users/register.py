...
async def message_commands_reg(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        msg_text = await generate_profile_register(message.from_user.id, data['dict_user_data'], "cmd_register_step_1")
        data['message_id'] = await message.answer(text=msg_text, parse_mode="HTML")
...
async def callback_commands_reg(call: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        msg_text = await generate_profile_register(call.message.chat.id, data['dict_user_data'], "cmd_register_step_1")
        data['message_id'] = await call.message.answer(text=msg_text, parse_mode="HTML")
        await UserRegister.user_input_name.set()